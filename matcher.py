...
class ToolMatcher:
    def __init__(self, embedding_model: str = "text-embedding-v4", dimensions: int = 1024, top_servers: int = 5, top_tools: int = 3):
    ...
    def match_servers(self, server_desc: str) -> List[Dict[str, Any]]:
        ...
        for server in self.servers_data:
            if "description_embedding" not in server:
                continue
            desc_similarity = self.cosine_similarity(query_embedding, server["description_embedding"])
            summary_similarity = 0
            if "summary_embedding" in server:
                summary_similarity = self.cosine_similarity(query_embedding, server["summary_embedding"])
        ...
        for server_info in server_list:
            ...
            for tool in server["tools"]:
                if "description_embedding" not in tool:
                    continue
                tool_similarity = self.cosine_similarity(query_embedding, tool["description_embedding"])
    ...
    def match(self, input_text: str) -> Dict[str, Any]:
        ...
        if not server_desc or not tool_desc:
            return {"success": False, "error": "No tool_assistant tag found or invalid format", "matched_servers": [], "matched_tools": []}
        try:
            ...
            return {
                "success": True, 
                "matched_tools": simplified_tools
            }
        except Exception as e:
            return {"success": False, "error": str(e), "server_description": server_desc, "tool_description": tool_desc, "matched_servers": [], "matched_tools": []}